...
class GitHubSpider(BaseSpider):
    ...
    # Topics related to passive income / side hustles
    SEARCH_TOPICS = [
        "passive-income",
        "side-hustle",
        "indie-hacker",
    ...
    async def fetch(self, limit: int = 20, **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            ...
            # Fetch topic-based repositories
            for topic in self.SEARCH_TOPICS[:3]:
                try:
                    topic_repos = await self._fetch_topic_repos(client, topic, limit // 3)
    ...
    async def _fetch_trending(
        ...
        Uses GitHub search API to find recently starred repos.
        """
        ...
        for item in items[:limit]:
            ...
            opp = {
                ...
                "verified": True,  # GitHub repos are real
                ...
                "age_days": 0,
                "platform_risk": False,
                "monthly_income": 0,  # Variable
                "hours_per_day": 5.0,  # Weekly average
    ...
    async def _fetch_topic_repos(
        self, client: httpx.AsyncClient, topic: str, limit: int
    ) -> List[Dict[str, Any]]:
